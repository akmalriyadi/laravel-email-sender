        ...
        <div class="grid grid-cols-4">
            <Card>
                <div class="flex space-x-2 items-center">
                    ...
                    <div>
                        <p class="text-lg">Total Template</p>
                        <p class="font-bold text-2xl">{{ data.paginate ? data.paginate.total : 0 }}</p>
                    </div>
                </div>
            </Card>
        </div>
        <Button type="link" link="/admin/templates/create" color="primary" className="mt-4">Create</Button>
        <Card class="mt-4">
...
import { useTemplateComposables, useResponseComposables } from '@/Composables'
import { Card, BasicTable, Button } from '@/Components'
import { ref } from 'vue'
import { router } from '@inertiajs/vue3'

const { data, fetchData, total, deleteData, fetchTrash, forceDelete, restoreData } = useTemplateComposables()
